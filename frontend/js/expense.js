...
function getBudgetMonthForExpense(expenseDate) {
    if (!expenseDate) {
        return null;
    }
...
function getBudgetForMonth(month) {
    ...
    return fetch(`http://127.0.0.1:5000/get-budget${monthQuery}`, { headers: getHeaders() })
        .then(res => {
            if (!res.ok) throw new Error('Budget check failed');
            return res.json();
        })
...
function addExpense() {
    ...
    const selectedBudgetMonth = getBudgetMonthForExpense(date);

    getBudgetForMonth(selectedBudgetMonth).then((budget) => {
        if (!budget || !budget.monthly_budget || Number(budget.monthly_budget) <= 0) {
            const budgetLabel = selectedBudgetMonth || 'this expense month';
            alert(`Please set a budget for ${budgetLabel} before adding this expense.`);
    ...
    .then(data => {
        ...
        const renderDayWiseExpenses = () => {
            ...
            document.querySelectorAll('.all-delete-btn').forEach(btn => {
                btn.addEventListener('click', function () {
                    showConfirm('Are you sure you want to delete this expense?', { okText: 'Delete', cancelText: 'Cancel' }).then(confirmed => {
                        if (!confirmed) return;
                    ...
                    .then(res => {
                        if (!res.ok) {
                            return res.json().then(errObj => {
                                throw new Error(errObj.message || 'Failed to delete expense');
                            });
                        }
                        return res.json();
                    })
                    ...
                    .catch(err => showPopup('Error: ' + err.message, 'error', 5000));
                });
            });
        });
        };
